...
<body>
	...
	$(document).ready(function() {		
		...
		$('#previous').click(function() { document.location = '${pageContext.request.contextPath}/admin/clients/clienthistory?id=${client.id}&page=${page == 1 ? 1 : page - 1}'; });
		$('#ok').click(function() { document.location = '${pageContext.request.contextPath}/admin/clients/clienthistory?id=${client.id}&page=' + parseInt($('#page').val()); });
		$('#next').click(function() { document.location = '${pageContext.request.contextPath}/admin/clients/clienthistory?id=${client.id}&page=${page + 1}'; });
		
		$(".orders-list > tbody tr")
			.css('cursor', 'pointer')
			.click(function(ev) {
				var target = '${pageContext.request.contextPath}/admin/clients/historydetails?id=' + $(this).attr('data-id');
	...
	<ul class="inline">
            <li><c:out value="${client.name} ${client.firstname}"></c:out> !!!</li>
             <li>Son solde est de :</li>
            <li class="label 
<c:choose>
	...
	<table class="table orders-list">
		...
		<tbody>
			...
			<c:forEach items="${lbalancehistory}" var="balanceHistory">
				<tr data-id="<c:out value="${balanceHistory.operationId}"></c:out>">
					<td><c:out value="${balanceHistory.id}"></c:out></td>
